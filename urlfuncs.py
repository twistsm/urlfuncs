...
POPULAR_ENCODINGS = ('utf-8', 'cp1251', 'ascii', ' ISO-8859-1')
...
def decode_string(string):
    ...
    try:
        ...
        if enc['encoding'] == 'ascii':
            ...
            unicode_string = string.decode(enc['encoding'])
    except:
        pass
...
def decode_url(url):
    ...
    parsed = urlparse.urlparse(decoded_string)
    if parsed.netloc:
...
def urlencode_string(string):
    ...
    """
    if type(string) is unicode:
        quoted_string = urllib.quote_plus(string.encode('utf-8'))
    else:
        quoted_string = urllib.quote(string)
...
def make_absolute_url(relative, baseurl):
    ...
    absolute_url = urlparse.urljoin(baseurl, relative)
    return absolute_url
...
def parse_url_list(text):
    """ Parse urls from text. Raises Exception if any of rows doesn't contain url.
    ...
    for url in urls:
        i += 1
        if not is_string_url(url):
            raise ValueError('некорректный URL %s на строке %s' % (url.encode('utf-8'), i))
            break
    return urls

def parse_domain_list(text):
    """ Parse domains from text. Raises Exception if any of rows doesn't contain domain.

    :param text: String text with domains in every line
    ...
    for domain in domains:
        ...
        if not is_string_domain(domain):
            raise ValueError('некорректный домен %s на строке %s' % (domain.encode('utf-8'), i))
            break
    return domains
